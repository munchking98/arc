...
const pepiForm = document.querySelector('.pepi');
document.querySelector('.pepi-btn').addEventListener('click', () => {
  ...
  pepiForm.appendChild(div);
});
const usrId = [];
const usrPw = [];
pepiForm.addEventListener('submit', async (e) => {
  try {
    e.preventDefault();
    ...
    // await axios.post('/pepi', { pepiId, pepiPwd });
  } catch (err) {
    console.error(err);
  }
});
